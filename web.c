...
#define th_hi()ST_MAX 16
static char wb_history[th_hi()ST_MAX][256];
...
static void wb_history_load(void) {
    ...
    buf[n] = 0;
    char *p = buf;
    while (*p && wb_history_n < th_hi()ST_MAX) {
        char *eol = strchr(p, '\n');
        if (eol) *eol = 0;
        if (*p) {
...
static void wb_history_add(const char *url) {
    ...
    if (wb_history_n < th_hi()ST_MAX) wb_history_n++;
    ...
    if (!d) {
        ...
        while ((e = readdir(d))) {
            ...
            snprintf(line, sizeof(line), "%-13s %-8s %-8s %s",
                     ...
                     car?"YES":"no",
                     up?"UP":"DOWN");
            if (car) wb_w(th_grn()); else if (wifi) wb_w(th_dim()2); else wb_w(th_fg());
            wb_w(line);
            y++;
            if (y >= rows-3) break;
        }
...
static void render_text(const char *text, int rows, int cols) {
    wb_paint_bg(rows, cols);
    wb_at(1, 2);
    wb_w(th_yel()); wb_w("\x1b[1m── PAGE ──"); wb_w("\x1b[22m");
    ...
    wb_at(rows-1, 2);
    wb_w(th_dim() "Press any key to return");
    fflush(stdout);
    unsigned char c; read(0,&c,1);
}
