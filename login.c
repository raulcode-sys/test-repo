...
static int lg_do_login(int rows, int cols) {
    ...
    lg_draw_screen(rows, cols, "── LOGIN ──");
    lg_at(12, cx); lg_w(th_dim()2"Username: "); lg_w(th_fg()); lg_w(uname);
...
static void lg_do_register(int rows, int cols) {
    ...
    lg_draw_screen(rows, cols, "── CREATE ACCOUNT ──");
    lg_at(12, cx); lg_w(th_dim()2"Username: "); lg_w(th_fg()); lg_w(uname);
    lg_at(13, cx); lg_w(th_dim()2"Password: "); lg_w(th_fg()); lg_w("****");
...
static void lg_run(void) {
    ...
    if (g_nusers == 0) {
        ...
        if (!g_persist_ok) {
            ...
            lg_at(14, cx); lg_w(th_dim()2"Accounts will be lost on reboot.");
            lg_at(15, cx); lg_w(th_dim()2"Run setup-persist from shell to set up USB storage.");
        }
        fflush(stdout);
        sleep(2);
    ...
    while (1) {
        ...
        lg_at(12, cx); lg_w(th_fg()); lg_w("\x1b[1mL\x1b[22m"); lg_w(th_dim()2"  Login");
        lg_at(13, cx); lg_w(th_fg()); lg_w("\x1b[1mN\x1b[22m"); lg_w(th_dim()2"  New account");
        lg_at(14, cx); lg_w(th_fg()); lg_w("\x1b[1mG\x1b[22m"); lg_w(th_dim()2"  Continue as guest (no persistence)");
